...
from klimatika_frontend.components.filters import date_period, load_stations, multiselect_stations, select_aggregation
...
from klimatika_frontend.state.session import init_session_state, remember_selection, require_auth
from klimatika_frontend.utils.formatters import station_id, station_label
...
def _render_top_navigation() -> None:
    ...
    columns = st.columns([1.18, 1, 1, 1, 1.28, 1, 1, 1], gap="small")
    with columns[0]:
        if st.button("На главную", key="top_nav_home", use_container_width=True):
            st.switch_page("app.py")

    for column, item in zip(columns[1:], TOP_NAVIGATION):
        with column:
